...
transformed as(
    select 
        ...
        PARSE_TIMESTAMP('%Y-%m-%dT%H:%M:%SZ', soh.data_vencimento) AS data_vencimento,		
        soh.frete,		
        PARSE_TIMESTAMP('%Y-%m-%dT%H:%M:%SZ', data_embarque) AS data_embarque,		
        subtotal,		
        soh.vencimento_total,		
        PARSE_TIMESTAMP('%Y-%m-%dT%H:%M:%SZ', data_pedido) AS data_pedido,		
        soh.id_cliente,		
        soh.id_territorio,		
